...
@Component({
	selector: 'util-route-nav',
	templateUrl: './route-nav.component.html',
	styleUrls: ['./route-nav.component.css']
})
export class UtilRouteNavComponent implements OnInit, OnDestroy {

	@HostBinding('class') navClass = 'm-left-nav';
	@Input() navList: Array<RouterConfig>;
	@Input() isOnlyIcon: boolean;
	@Input() canToggleIcon = true;
	/** Emits when the toggle only show icon changes */
	@Output() onlyIconChange: EventEmitter<boolean> = new EventEmitter();
	...
	toggleOnlyIcon(): void {
		console.log(this.canToggleIcon);
		console.log(this.isOnlyIcon);
		if (this.canToggleIcon) {
			this.isOnlyIcon = !(!!this.isOnlyIcon);
			Promise.resolve(null)
				.then(() => {
					this.onlyIconChange.emit(this.isOnlyIcon);
				});
		}
	}
}
